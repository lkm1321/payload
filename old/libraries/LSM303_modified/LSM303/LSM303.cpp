...
{
  Wire1.beginTransmission(acc_address);
  Wire1.write(reg);
  Wire1.write(value);
...
byte LSM303::readAccReg(byte reg)
{
  ...
  Wire1.write(reg);
  last_status = Wire1.endTransmission();
  Wire1.requestFrom(acc_address, (byte)1);
  value = Wire1.read();
...
{
  Wire1.beginTransmission(mag_address);
  Wire1.write(reg);
  Wire1.write(value);
  last_status = Wire1.endTransmission();
}
...
byte LSM303::readMagReg(int reg)
{
  ...
  Wire1.write(reg);
  last_status = Wire1.endTransmission();
  Wire1.requestFrom(mag_address, (byte)1);
  value = Wire1.read();
  Wire1.endTransmission();
...
void LSM303::readAcc(void)
{
  ...
  Wire1.write(OUT_X_L_A | (1 << 7));
  last_status = Wire1.endTransmission();
  Wire1.requestFrom(acc_address, (byte)6);
...
void LSM303::readMag(void)
{
  ...
  Wire1.write((_device == device_D) ? translated_regs[-OUT_X_L_M] | (1 << 7) : translated_regs[-OUT_X_H_M]);
  last_status = Wire1.endTransmission();
  Wire1.requestFrom(mag_address, (byte)6);
